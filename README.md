...
S3-Dock is an automated, high-fidelity computational structural biology pipeline designed to evolve, fold, dock, simulate, and validate optimized peptide candidates against challenging multi-chain oligomeric complexes (e.g., amyloid fibrils) or globular receptor interfaces (e.g., MDM2).

The platform seamlessly bridges evolutionary sequence design, deep-learning structural prediction, blind macro-docking, and explicit solvent molecular dynamics into a unified, fault-tolerant workflow.
...
Utilizes an evolutionary Monte Carlo simulated annealing algorithm. It designs sequences matching specific secondary structure propensities (helices/sheets) under rigid hydrophobic/polar (H/P) patterns, optimized for target net charge, Boman solubility indices, and hydrophobic moments.

**Phase 2: Structural Prediction & Filtering** (`02_predict_struct.py`)
Triggers machine-learning-driven 3D structural folding (via Boltz-2). Implements a sequence-validated, fault-tolerant, chain-agnostic local pocket alignment filter to automatically discard misfolded candidates without manual chain tracking.

**Phase 3: Blind Docking** (`03_blind_dock.py`)
Prepares target and ligand topologies and performs blind rigid-body ensemble docking against the localized receptor pocket.
...
- 🕸️ **`[Seed]_interaction_network.png`** — A universal bipartite interaction network wiring diagram mapping atomic contacts (<4.5 Å) between the peptide and the target receptor.
