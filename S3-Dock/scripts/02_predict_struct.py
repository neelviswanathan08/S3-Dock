...
for idx, (seq_id, sequence) in enumerate(jobs, 1):
    ...
    if 'pocket_contacts' in config and config['pocket_contacts']:
        yaml_lines.append("\nconstraints:\n  - pocket:\n" + f"      binder: {binder_id}\n      contacts:")
        for contact in config['pocket_contacts']:
            yaml_lines.append(f"        - [{contact[0]}, {contact[1]}]")
            ...
            for file in files:
                if file.endswith(".json") and "confidence" in file.lower():
                    try:
                        ...
                        if "models" in data and len(data["models"]) > int(model_num):
                            iptm_score = float(data["models"][int(model_num)].get("iptm", 0.0))
                        elif f"model_{model_num}" in data:
                            iptm_score = float(data[f"model_{model_num}"].get("iptm", 0.0))
                        elif "model_0" in data:
                            iptm_score = float(data["model_0"].get("iptm", 0.0))
                    except: pass
